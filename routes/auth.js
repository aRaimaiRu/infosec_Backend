...
router.post(
  '/register/shop',
  [authorize(), CheckAuthorizeWithTable('shops', 1, 1), registerShopSchema],
  registerShop
); //FE ->register as User -> login ->register as shop
...
function refreshTK(req, res, next) {
  ...
  userService
    .refreshJWT(req.signedCookies.refreshToken)
    .then((user) => {
      let { refreshtoken, ...userobj } = user;
  ...
  userService
    ...
    .then(async (user) => {
      let info = await transport.sendMail({
        ...
        html: `Please click button to Verify Email ${
          process.env.CURRENTURL + '/api/user/activate/' + user.id
        }`, // html body
      });
...
function update(req, res, next) {
  if (req.params.id != req.user.id)
    return res.json({ message: 'Unauthorized Failed to Update' });
  userService
...
function _delete(req, res, next) {
  if (req.params.id != req.user.id)
    return res.json({ message: 'Unauthorized Failed to Delete' });
  userService
...
function resendVerificationmail(req, res, next) {
  userService
    .reSendVerifyEmail({ username: req.body.username })
    .then(async (user) => {
      let info = await transport.sendMail({
        from: process.env.SENDEREMAIL, // อีเมลผู้ส่ง
        to: req.body.username, // อีเมลผู้รับ สามารถกำหนดได้มากกว่า 1 อีเมล โดยขั้นด้วย ,(Comma)
        subject: 'Verification', // หัวข้ออีเมล
        text: 'Please click button to Verify Email', // plain text body
        html: `<p>Please click to Verify Email </p> ${
          process.env.CURRENTURL + '/api/user/activate/' + user.id
        }`, // html body
      });
      console.log('send verify Email complete');
    ...
    .then((user) =>
      res.json({ message: 'Resend Verification Email Successfully' })
    )
    .catch(next);
}
...
async function sendforgetpasswordemail(req, res, next) {
  try {
    ...
    const token = await userService.forgotpassword({
